...
test.serial("Writing a key", async (t) => {
    t.plan(2);
    const msg = await promisifyMonitorStream(kvMonitor, "success", MONITOR_STREAM_EVENT_EMIT_TIMEOUT_MS, {
...
test("Remove a non-existent namespace", async (t) => {
    ...
    t.plan(2);
    const msg = await promisifyMonitorStream(kvMonitor, "err", MONITOR_STREAM_EVENT_EMIT_TIMEOUT_MS, {
